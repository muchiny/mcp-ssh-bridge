...
mod tests {
    ...
    fn test_build_command_defaults() {
        let args: SshPodmanExecArgs = serde_json::from_value(
            json!({"host": "s", "container": "myapp", "command": "ls"}),
        )
        .unwrap();
    ...
    async fn test_full_pipeline_success() {
        ...
        let result = handler
            .execute(Some(json!({"host": "server1", "container": "mycontainer", "command": "ls"})), &ctx)
            .await
            .unwrap();
